...
- **Persistent user memory:** durable per-user memory in SQLite — quiet flags, custom instructions, and learned facts that survive restarts.
- **Per-user quiet mode:** tell the bot "diam" (or "shut up") and it stops responding *to you* across mentions, prefix, and slash commands until you say "ngomong lagi" (or "speak again"). Other users are unaffected.
- **Custom instructions:** `!memory set <teks>` stores personal directives (nickname, answer style, language preference) that the AI must follow in every future reply. `!memory show`, `!memory clear`, `!memory facts`, and `!memory forget` manage the store.
- **Passive fact learning:** the bot extracts durable facts from conversations (hobbies, work schedule, preferences) with a cheap background AI call — deduplicated, capped at 50 per user with least-recently-used eviction, and recalled by relevance when answering.
...
- **Personalized responses:** interaction preferences (response length, language) auto-adapt per user and influence future responses.
...
| Memory | `!memory set <teks>`, `!memory show`, `!memory clear`, `!memory facts`, `!memory forget` |
| Quiet mode | `!diam` (mute the bot for you), `!ngomong lagi` (unmute) |
...
```text
@bot diam                          → bot goes quiet for you only
@bot ngomong lagi                  → bot resumes talking to you

!memory set Panggil gue Ani, jawab singkat pake bahasa santai
!memory set I'm a night-shift worker — keep answers short after midnight
!memory facts                      → shows what the bot has learned about you
!memory forget                     → wipes learned facts
```
...
- SQLite user memory: `data/user-memory.db` (quiet flags, custom instructions, learned facts)
- JSON state for AFK, user preferences, learned patterns, wake/sleep state, and server settings.
- Automatic backups for supported JSON stores and the user-memory database.
...
| `USER_MEMORY_DB_PATH` | `./data/user-memory.db` | SQLite persistent user memory (quiet flags, custom instructions, learned facts). |
...
└── utils/                   Memory (conversation + persistent user facts), AFK, reminders,
                             warnings, security, settings, backups, reaction roles,
                             VoiceMaster, rate limits, logging, and health
```
...
The test suite covers the provider router, input sanitizer, security helpers, learned patterns, persistent user memory (quiet mode, custom instructions, fact extraction/recall), warnings, AFK behavior, voice playback, integration handlers, performance/regression cases, and the midnight restart supervisor.
...
Bot ini adalah asisten Discord AI multifungsi dengan **Jarvis Mode**, chat dan tanya jawab, pencarian web dengan sumber, voice/TTS, moderasi, AFK, reminder SQLite, **memori pengguna persisten** (mode diam per-user, instruksi personal via `!memory`, pembelajaran fakta otomatis), self-learning, VoiceMaster, reaction roles, dan routing AI multi-provider dengan failover otomatis.
