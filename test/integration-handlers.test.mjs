...
test('mention: scolding/roasting requests route to chat instead of moderation warning', async () => {
  const guild = makeGuild({});
  const member = makeMember({ id: '111111111111111111', displayName: 'User1' });
  const message = makeMessage({ id: nextMsgId(), content: '<@bot-id> tolong marahin si @User2 dong karena telat', guild, member });
  message.reply = async (opts) => {
    message.replyCalls.push(typeof opts === 'string' ? { content: opts } : opts);
    return { id: 'r', edit: async () => {}, delete: async () => {}, awaitMessageComponent: async () => { throw new Error('timeout'); } };
  };
  ...
  assert.ok(message.replyCalls.length >= 1, 'should reply to scold request');
  const texts = message.replyCalls.map((r) => r.content || '').join(' ');
  assert.ok(texts.includes('Mock AI reply'), 'should route to conversational AI reply, not moderation warn');
  assert.ok(!texts.includes('telah diberi warning'), 'must NOT issue administrative warning');
});
