...
test('smoke: all commands modules expose .data and .execute', async () => {
  const cmds = ['ask', 'chat', 'summarize', 'help', 'admin', 'ping', 'weather', 'invite', 'reactionrole', 'test-ai'];
...
test('smoke: deploy-commands registers exactly 10 commands', async () => {
  const { REST, Routes } = await import('discord.js');
  const files = ['ask', 'chat', 'summarize', 'help', 'admin', 'ping', 'weather', 'invite', 'reactionrole', 'test-ai'];
