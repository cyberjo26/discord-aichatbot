...
import { closeUserMemory } from './utils/user-memory.js';
...
client.once(Events?.ClientReady || 'clientReady', async () => {
  ...
  try {
    const deploy = await registerCommands(client.user?.id);
...
function shutdown(signal, exitCode = 0) {
  ...
  try {
    ...
    logger.error(`Failed to close user memory database: ${err.message}`);
  }
  client.destroy();
