...
import { chatCompletion, getAiStats, getProvider } from './ai/router.js';
...
import { isQuiet, setQuiet, detectQuietCommand, buildMemoryInjection, extractFacts, getCustomInstructions, setCustomInstructions, listFacts, clearFacts } from './utils/user-memory.js';
...
export async function handlePrefixCommand(message) {
  ...
  // Per-user quiet gate. Intent is checked on the COMMAND TOKEN only, never on
  // args — otherwise `!memory set jangan diam` or `!chat gue lagi diam` would
  // be misread as quiet directives. Bare `!diam` mutes; `!ngomonglagi` unmutes.
  // Per-user quiet gate. Exact-command matcher on the command head only —
  // `!memory set jangan diam` or `!ask gue lagi diam` must not toggle state.
  const headPhrase = content.split(/\s+/).slice(0, 2).join(' ');
  const quietIntent = detectQuietCommand(headPhrase);
  if (quietIntent === 'quiet' && !isOwner(message.author.id)) {
    setQuiet(message.author.id, true);
    return message.reply('Oke, diam. 🙂 Bilang "ngomong lagi" (atau tag aku) kalau mau aku balas.');
  }
  if (quietIntent === 'unquiet' && isQuiet(message.author.id)) {
    setQuiet(message.author.id, false);
    return message.reply('Oke, gue ngomong lagi! 🗣️');
  }
  if (isQuiet(message.author.id)) return;
  ...
  switch (command) {
    ...
    case 'memory':
      return handleMemory(message, args);
    ...
    case 'test-ai':
    case 'test-api':
      return handleTestAi(message, args);
...
async function handleAsk(message, query, mode) {
  ...
  try {
    ...
    if (fresh.usedFreshData) {
      ...
      if (mode === 'voice') {
        await handleVoiceReply(message, fresh.answer, replyOptions);
      }
    ...
    const answer = await chatCompletion([
      { role: 'system', content: buildJarvisPrompt({ memoryInjection: buildMemoryInjection(message.author.id, query) }) },
      { role: 'user', content: query },
    ]);
    void extractFacts(message.author.id, [{ role: 'user', content: query }, { role: 'assistant', content: answer }]).catch(() => {});
...
// Per-user persistent memory: custom instructions the AI must follow.
async function handleMemory(message, args) {
  ...
  switch (sub) {
    case 'set': {
      if (!payload) {
        return message.reply('❗ Tulis instruksinya. Contoh: `!memory set Jawab singkat, panggil gue Ani`');
      }
      const saved = setCustomInstructions(message.author.id, payload);
      return message.reply(`💾 Oke, gue ingat: *"${saved}"* — bakal gue ikutin mulai sekarang.`);
    }
    case 'show': {
      const text = getCustomInstructions(message.author.id);
      return message.reply(text ? `📋 Instruksimu: *"${text}"*` : '📋 Belum ada instruksi khusus. Set pakai `!memory set <teks>`.');
    }
    case 'clear':
      setCustomInstructions(message.author.id, '');
      return message.reply('🗑️ Instruksi pribadimu udah gue hapus.');
    case 'facts': {
      const facts = listFacts(message.author.id);
      if (!facts.length) return message.reply('🧠 Belum ada fakta yang gue pelajari dari obrolanmu.');
      return message.reply(
        `🧠 ${facts.length} fakta yang gue ingat:\n` +
        facts.slice(0, 15).map((f) => `• ${f.fact}`).join('\n')
      );
    }
    case 'forget':
      clearFacts(message.author.id);
      return message.reply('🧹 Semua fakta tentangmu udah gue hapus.');
    default:
      return message.reply(
        '📖 **!memory** — atur ingatan pribadimu:\n' +
        '`!memory set <teks>` — instruksi yang harus gue ikutin (mis. gaya jawab, nama panggilan)\n' +
        '`!memory show` — lihat instruksimu\n' +
        '`!memory clear` — hapus instruksimu\n' +
        '`!memory facts` — fakta yang gue pelajari dari obrolan\n' +
        '`!memory forget` — hapus semua fakta'
      );
  }
}
...
async function handleChat(message, text, mode) {
  ...
  try {
    ...
    const systemPrompt = buildJarvisPrompt({ memoryInjection: buildMemoryInjection(message.author.id, text) });
    const messages = [
    ...
    void extractFacts(message.author.id, [{ role: 'user', content: text }, { role: 'assistant', content: answer }]).catch(() => {});
...
async function handleTestAi(message, args) {
  // Diagnostic fires real API calls — owner or server admin only (matches /test-ai)
  const isTestAiAllowed =
    isOwner(message.author.id) ||
    (message.member && message.member.permissions.has(PermissionFlagsBits.Administrator));
  if (!isTestAiAllowed) {
    return message.reply('🔒 Perintah ini hanya untuk admin server atau owner bot.');
  }

  if (!args) {
    return message.reply('❗ Gunakan: `!test-ai <provider> [prompt]`\nContoh: `!test-ai 9router Halo!` atau `!test-ai groq Halo!` atau `!test-ai gemini`');
  }

  const parts = args.trim().split(/\s+/);
  const providerName = parts[0].toLowerCase();
  const testPrompt = parts.slice(1).join(' ') || 'Tes koneksi dan latency. Balas satu kalimat singkat.';

  const provider = getProvider(providerName);
  if (!provider || !provider.enabled()) {
    return message.reply(`❌ Provider **[${providerName}]** belum aktif atau belum dikonfigurasi di \`.env\`.\nPastikan Base URL dan API Key sudah diisi.`);
  }

  const statusMsg = await message.reply(`⏳ Menghubungi provider **${providerName}**...`);
  ...
  try {
    ...
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: '💬 Input Prompt', value: `\`\`\`\n${testPrompt.slice(0, 300)}\n\`\`\``, inline: false },
        { name: '📤 Respon Provider', value: `\`\`\`\n${(result.text || '(kosong)').slice(0, 900)}\n\`\`\``, inline: false }
      )
      .setTimestamp();
