...
async function createTempChannel(state) {
  ...
  try {
    ...
    // Move the user to the new channel. If the move fails (user left the hub
    // mid-create, bot lacks MoveMembers), the channel is empty and no
    // voiceStateUpdate will ever fire for it — delete it right away instead
    // of leaving an orphan until the next restart cleanup.
    try {
      await member.voice.setChannel(tempChannel);
      logger.info(`🔊 VoiceMaster: Created "${channelName}" for ${member.displayName}`);
    } catch (moveErr) {
      logger.warn(`VoiceMaster move failed, deleting empty temp channel: ${moveErr.message}`);
      getGuildChannels(guild.id).delete(tempChannel.id);
      saveTempChannels(guild.id);
