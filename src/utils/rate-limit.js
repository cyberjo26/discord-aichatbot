 ...
 *
 * All limits are evaluated read-only first; the user/guild counters are only
 * incremented once every check has passed (evaluate-then-commit). This keeps a
 * denied request from consuming another quota's slot — previously the guild
 * counter was incremented before the user quota was checked, so one user
 * hammering past their 20/min could silently starve the guild's shared
 * 150/min allowance.
 *
...
export function checkRateLimit(userId, guildId) {
  ...
  // 2. Evaluate Guild Limit (read-only — no increment yet)
  let gLimit = null;
  if (guildId) {
    gLimit = guildLimits.get(guildId);
  ...
  // 3. Evaluate User Limit (read-only — no increment yet)
  ...
  // 4. All checks passed — commit both counters and issue the token
  if (gLimit) gLimit.count++;
  uLimit.count++;
