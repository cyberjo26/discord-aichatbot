...
export async function backupDataFiles() {
  ...
  if (config.userMemoryDbPath) filesToBackup.push(config.userMemoryDbPath);
  ...
  try {
    ...
    logger.error(`Cannot create backup directory, skipping backup: ${err.message}`);
    return;
  }
  ...
  for (const file of filesToBackup) {
    if (fs.existsSync(file)) {
      try {
        ...
        } else if (file === config.userMemoryDbPath) {
          // SQLite snapshot: copy db + -wal together — SQLite recovers a
          // consistent db from the pair (WAL replay on next open).
...
export function initBackups() {
  // Run once immediately (rejection-safe — an escaped rejection here would
  // hit the global unhandledRejection handler and skip the daily schedule),
  // then daily.
  Promise.resolve(backupDataFiles()).catch((err) =>
    logger.error(`Initial backup failed: ${err.message}`)
  );

  const DAILY_MS = 24 * 60 * 60 * 1000;
  const timer = setInterval(() => {
    backupDataFiles().catch((err) => logger.error(`Daily backup failed: ${err.message}`));
  }, DAILY_MS);
  timer.unref();
}
