...
export async function buildLearnedKnowledge(userQuery) {
  ...
  // Track usage so the 500-cap eviction ("remove least used") ranks by real
  // usage instead of evicting the oldest pattern (all-zero usageCount).
  for (const p of matchedPatterns) {
    markPatternUsed(p.trigger);
  }
