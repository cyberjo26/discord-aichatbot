...
if (!hasConfiguredAi) {
  console.error('Missing AI provider key: isi salah satu dari OPENROUTER_API_KEY, GEMINI_API_KEY, GROQ_API_KEY, CEREBRAS_API_KEY, PUTER_API_KEY, atau CUSTOM_AI_BASE_URL.');
  process.exit(1);
}
...
const config = {
  ...
  geminiApiKeys: envList('GEMINI_API_KEYS').length > 0
    ? envList('GEMINI_API_KEYS')
  ...
  groqApiKeys: envList('GROQ_API_KEYS').length > 0
    ? envList('GROQ_API_KEYS')
  ...
  warningsFile: process.env.WARNINGS_FILE || './data/warnings.json',
  ...
  userMemoryDbPath: process.env.USER_MEMORY_DB_PATH || './data/user-memory.db',
