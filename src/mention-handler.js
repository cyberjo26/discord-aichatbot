...
import { freshAnswer, looksTimeSensitive } from './ai/fresh.js';
...
export async function handleMention(message) {
  ...
  try {
    ...
    if (!rawContent) {
      if (isBotAwake() && !isQuiet(message.author.id)) await message.reply('Hai! Ada yang bisa aku bantu? 🤖');
      return;
    }

    // Handle manual learn trigger: belajar: or ajarkan:
    // Gated: learned patterns are injected into future AI routing prompts, so
    // allowing anyone to teach would let a bad actor poison the bot's knowledge.
    const normalizedRaw = rawContent.toLowerCase();
    const canTeach =
      isOwner(message.author.id) ||
      (message.member && message.member.permissions.has(PermissionFlagsBits.ManageGuild));
    if ((normalizedRaw.startsWith('belajar:') || normalizedRaw.startsWith('ajarkan:')) && canTeach) {
    ...
    // Per-user quiet mode: "diam" mutes the bot for THIS user only, in every
    // entry point, until they explicitly ask it to speak again. Only a short
    // explicit directive counts — a tagged question that happens to contain
    // the word "diam" must not toggle state.
    const quietIntent = rawContent.trim().length <= 40 ? detectQuietIntent(rawContent) : null;
    if (quietIntent === 'quiet' && !isOwner(userId)) {
      setQuiet(userId, true);
      await message.reply('Oke, diam. 🙂 Tag aku lagi dan bilang "ngomong lagi" kalau mau aku balas.');
      return;
    }
    if (quietIntent === 'unquiet' && isQuiet(userId)) {
      setQuiet(userId, false);
      await message.reply('Oke, gue ngomong lagi! 🗣️');
      return;
    }
    if (isQuiet(userId)) {
      // Muted and this isn't an unmute directive — stay silent.
      return;
    }
    ...
    if (plan.action === 'chat' || plan.action === 'knowledge' || plan.action === 'code_help') {
      // Chat questions anchored to recent time ("hari ini", "terbaru", "2026")
      // are upgraded to the fresh pipeline — the classifier may have missed
      // them, but the model's knowledge cutoff still makes them risky.
      if (plan.action === 'chat' && looksTimeSensitive(rawContent)) {
        logger.info(`[Step 2] 🕒 chat → knowledge (time-sensitive anchor detected)`);
    ...
    pendingReply = await message.reply('⏳ Oke, saya periksa dulu...').catch(() => null);
    ...
    logger.info(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
    // Clean up the "checking..." placeholder so it doesn't linger forever
...
const SCOLD_HINT = /\b(marahi|marahin|omeli|omelin|scold|roast|sindir|ejek)\b/i;
...
function fastRoute(rawContent) {
  ...
  // Verbal scolding/roasting is chat banter, not administrative moderation warning
  if (SCOLD_HINT.test(text)) {
    return { thought: 'verbal scold/banter -> chat', action: 'chat', params: {}, response_style: 'playful', rawQuery: rawContent };
  }
...
async function executeAction(plan, message) {
  ...
  switch (action) {
    ...
    case 'ask_clarification': {
      // Learning capture is gated (same policy as belajar:/ajarkan:): the
      // clarification question is still asked, but only privileged users'
      // explanations are persisted as routing knowledge.
      const canLearn =
        isOwner(message.author.id) ||
        (message.member && message.member.permissions.has(PermissionFlagsBits.ManageGuild));
      if (canLearn) {
...
async function generateNaturalResponse(plan, result, message) {
  ...
  if (plan.action === 'chat' || plan.action === 'knowledge' || plan.action === 'code_help') {
    ...
    const systemPrompt = buildJarvisPrompt({
      ...
      memoryInjection: buildMemoryInjection(userId, rawQuery),
    ...
    // Knowledge questions may exceed the model's knowledge cutoff — run the
    // fresh-answer pipeline (Find → Compare → Select → Connect → Conclude)
    // first; fall back to internal knowledge when the gate says the question
    // is evergreen or no web data is found.
    if (plan.action === 'knowledge') {
      const ctx = getContext(userId);
      const fresh = await freshAnswer(rawQuery, {
        conversationContext: [ctx.contextSummary, ...ctx.topics].filter(Boolean).join(' — ').slice(0, 500),
      });
      if (fresh.usedFreshData) {
        addMessage(userId, 'user', rawQuery);
        addMessage(userId, 'assistant', fresh.answer);
        const sourceLine = fresh.sources.length > 0
          ? `\n\n📚 **Sumber terbaru:**\n${fresh.sources.map((s) => `• ${s.title} — ${s.url}`).join('\n')}`
          : '';
        const caveat = fresh.confidence === 'low' ? '\n\n*(info dari web masih tipis/bertentangan — hati-hati ya)*' : '';
        return `${fresh.answer}${sourceLine}${caveat}`;
      }
    }
    ...
    // Detached: the 60 s idle button wait must not hold the mention's
    // rate-limit token (50 concurrent waits would lock out the whole bot).
    // Return null — sendWithArticleButton sends its own reply, so the
    // chat/knowledge branch must not reply a second time.
    if (plan.action === 'knowledge') {
      void sendWithArticleButton(message, answer, rawQuery, userId).catch((e) =>
        logger.error(`Article button error: ${e.message}`)
      );
      return null;
    }
  ...
  // Passive learning: extract durable facts from the recent exchange.
  // Fire-and-forget — must not block the reply or hold the rate-limit token.
  void extractFacts(userId, [{ role: 'user', content: rawQuery }, { role: 'assistant', content: response }]).catch(() => {});
  return response;
}
